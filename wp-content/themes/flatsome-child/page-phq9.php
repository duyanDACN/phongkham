<?php
/* Template Name: Bảng sàng lọc SKTT PHQ-9 */
get_header();
?>
  ...
  <div class="col medium-12 small-12 large-12">
    <div class="header-sktt-gad">
      <div class="box-header text-center">
        ...
        <h3>BẢNG SÀNG LỌC SỨC KHOẺ TINH THẦN <strong>(PHQ-9)</strong></h3>
      </div>
    ...
    <div class="box-wh">
      ...
      $questions = [
          ...
          'Cảm thấy suy sụp, chán nản hoặc tuyệt vọng',
          'Khó vào giấc ngủ, khó duy trì giấc ngủ hoặc ngủ quá nhiều',
          'Cảm thấy mệt hoặc ít năng lượng',
          'Chán ăn hoặc ăn quá nhiều',
          'Cảm thấy bản thân mình tệ hại, thất bại, khiến bạn thân hoặc gia đình thất vọng',
          'Gặp vấn đề trong việc tập trung, ví dụ: khi đọc báo hoặc xem tivi',
          'Vận động hoặc nói ít hơn đến mức người khác nhận thấy sự khác biệt; hoặc ngược lại, cảm thấy bồn chồn, bứt rứt đến mức đi tới lui nhiều hơn bình thường',
          'Có suy nghĩ rằng: mình nên chết là tốt nhất hoặc suy nghĩ về việc làm tổn thương bản thân'
        ];
...
<script>
  ...
  function updateTotal() {
    ...
    if (answered === 9) viewResultBtn.classList.add("show");
  ...
  document.getElementById('btnViewResult').addEventListener('click', function () {
    ...
    function calculateLevel() {
      let phq9 = 0;
      for (let i = 1; i <= 9; i++) {
        phq9 += parseInt(document.querySelector(`input[name="q${i}"]:checked`)?.value || 0);
      }

      let phqLevel = '';
      if (phq9 >= 15) {
        phqLevel = `Mức độ trầm cảm nặng`;
      } else if (phq9 >= 10) {
        phqLevel = `Mức độ trầm cảm trung bình`;
      } else if (phq9 >= 5) {
        phqLevel = `Mức độ trầm cảm nhẹ`;
      } else {
        phqLevel = `Không trầm cảm`;
      }
      return { phq9, phqLevel };
    }
